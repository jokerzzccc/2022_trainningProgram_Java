...
public class Topic63 {
    public static void main(String[] args) {
        ...
        HashMap<String, Info> infoHashMap = new HashMap<>();
        String[] all = split[0].split(",");
        for (int i = 0; i < all.length; i++) {
            String[] char_count = all[i].split(":");
            String c = char_count[0];
            infoHashMap.put(c, new Info(c, i, Integer.parseInt(char_count[1])));
        }
        ...
        if (split.length > 1) {
            for (String s : split[1].split(",")) {
                ...
                String c = char_count[0];
                Info value = infoHashMap.get(c);
                value.count -= Integer.parseInt(char_count[1]);
                infoHashMap.put(c, value);
            }
        }

        StringBuilder stringBuilder = new StringBuilder();

        infoHashMap.values().stream()
                .filter(x -> x.count > 0)
                ...
                }).forEach(x -> stringBuilder.append(x.c).append(":").append(x.count).append(","));
